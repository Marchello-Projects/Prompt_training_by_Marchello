...
def build_history_keyboard(page: int, has_prev: bool, has_next: bool):
    ...
    if has_prev:
        row.append(
            InlineKeyboardButton(text="⬅️ Prev", callback_data=f"history_page:{page-1}")
        )
    if page > 0:
        row.append(InlineKeyboardButton(text="⬆️ Top", callback_data="history_page:0"))
    if has_next:
        row.append(
            InlineKeyboardButton(text="➡️ Next", callback_data=f"history_page:{page+1}")
        )

    export = InlineKeyboardButton(
        text="📥 Export to Excel", callback_data="export_excel"
    )
...
def export_history_to_excel():
    ...
    with sqlite3.connect("app/database/data.db") as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT mode, prompt, created_at FROM History ORDER BY created_at DESC"
        )
        rows = cursor.fetchall()
...
async def handle_export_excel(callback: CallbackQuery):
    ...
    if os.path.exists(path):
        ...
        await callback.message.answer_document(
            file, caption="📄 Here is your exported history."
        )
    else:
